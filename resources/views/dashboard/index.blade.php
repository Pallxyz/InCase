...
<x-layouts.dashboard title="Dasbor — InCase">
    <div class="flex h-screen bg-background">
        ...
        <div id="mobile-sidebar-backdrop" onclick="toggleMobileSidebar()"
            class="fixed inset-0 z-30 hidden bg-slate-900/40 backdrop-blur-sm lg:hidden"></div>
            ...
            <div class="mx-auto max-w-6xl px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
                ...
                <div class="flex flex-wrap items-center justify-between gap-4">
                    ...
                    <div class="flex items-center gap-3">
                        <a href="{{ route('scan-history.index') }}"
                            ...
                            <div class="mt-4 flex flex-col gap-3">
                                @forelse ($missingItems as $item)
                                    <div class="flex items-center gap-3 rounded-xl bg-card px-4 py-3 shadow-sm">
                                        <span
                                            class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-warning/10 text-warning">
                                            <x-dynamic-component :component="'icon.' . ($categoryIcons[$item->category] ?? 'cube')" class="h-4 w-4" />
                                        </span>
                                        <div>
                                ...
                                <div class="relative flex gap-4">
                                    ...
                                    <div class="mb-5 flex-1">
                                        <div class="flex flex-wrap items-center gap-2">
                                            <span
                                                class="text-sm font-bold text-foreground">{{ \Carbon\Carbon::parse($subject->start_time)->format('H:i') }}</span>
                                            <span
                                                class="text-sm font-medium text-foreground">{{ $subject->name }}</span>

                                            @if (!empty($subject->homework))
                                                <span
                                                    class="inline-flex items-center gap-1 rounded-full bg-warning/10 px-2 py-0.5 text-[11px] font-semibold text-warning">
                ...
                <div
                    class="mt-6 flex items-center gap-4 rounded-[24px] border border-dashed border-border bg-card p-6 shadow-sm">
                    <span
                    ...
                    <div>
                        ...
                        <p class="text-xs text-muted-foreground">Segera hadir — rangkuman & saran otomatis buat
                            kesiapan tas kamu.</p>
                    </div>
                </div>
            </div>
